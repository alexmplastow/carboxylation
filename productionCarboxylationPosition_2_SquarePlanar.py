...
for i, xyzObject in enumerate(XYZobjects):
	...
	try:
		...
		xyzObject.printToFile(f"outputs_αR2_planar/ligands_{str(i).zfill(4)}.xyz")
		xyzObject.writeSanityRecord(f"outputs_αR2_planar/ligands_{str(i).zfill(4)}_sanityRecord.txt", hydrogenSkip = True)
